...
tabulate.and.print<-function(data=seq.subset,samples=1,groups=19,counts=3, category="Class",titulo=paste("Ascription of seqences to",category, sep=" "),n.cols=3, normalize=TRUE,reorder=FALSE,niveles,flip=FALSE,legend.pos="right",print=TRUE,extra=scale_fill_discrete(),with.levels=FALSE,lev.el=c("Vari_lac+Stig_eucl+A622","Teph_atr+Sky_teph+A280","Teph_atr+Mue_atr+A440","Rhi_geo+Endo_macro+A194","Leca_poly+Mue_pyg+A670","Leca_poly+Lich_lec+A434","Aspi_myr+Sag_fis+A608","Rhi_geo+N+A172","Psori_con+N+A476","Psori_con+N+A229","Leci_lap+N+A792","Leca_intr+N+A360","Can_vit+N+A138","Teph_atr+Tae_atr+A809","Rhi_geo+Mue_pyg+A405","Leci_lap+Mue_pyg+A636","Leca_poly+Lich_lec+A418","Leca_poly+Cer_epi+A482","Leca_bic+Arth_var+A832","Umb_cyl+N+A032","Teph_atr+N+A361","Leca_schw+N+A227","Leca_poly+N+A243","Leca_bic+N+A368","Aspi_myr+N+A420","Aca_fus+N+A623"))
{
				...
				if(with.levels==TRUE)
					{
						if (flip==TRUE)
						{
							p1<-ggplot(dc, aes(x = factor(Var1), levels=lev.el, y=Freq, fill=factor(Var2), order=as.numeric(factor(Var2)))) + 		geom_bar(stat="identity",position=("stack")) + coord_flip() + theme(legend.position=legend.pos, legend.text=element_text(size=6)) + guides(fill=guide_legend(ncol=n.cols)) + labs(x="" , y="", title=titulo) + extra
						}else{
							if (flip==FALSE)
								{
								p1<-ggplot(dc, aes(x = factor(Var1), levels=lev.el, y=Freq, fill=factor(Var2),order=as.numeric(factor(Var2)))) + 	geom_bar(stat="identity",position=("stack"))+ theme(axis.text.x=element_text(angle=90), 	legend.position=legend.pos, legend.text=element_text(size=6)) + guides(fill=guide_legend(ncol=n.cols))+ 	labs(x="" , y="", title=titulo) + extra
								}
							}
					}
			}
